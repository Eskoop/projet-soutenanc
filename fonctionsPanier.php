<!-- <?php   
	 ...
	 if (isset($_POST['produit_id'], $_POST['quantité']) && is_numeric($_POST['produit_id']) && is_numeric($_POST['quantité'])) {   
	     ...
	     $stmt = $pdoManga->prepare('SELECT * FROM produits WHERE id = ?');   
	 ...
	 if (isset($_POST['update']) && isset($_SESSION['panier'])) {   
        /* Boucle à travers les d<<<<<<<<<<<<<<<<onnées postales afin de mettre à jour les quantités pour chaque produit du panier.*/   
